...
import (
	...
	"rawvelo/internal/pkg/hash"
	"rawvelo/internal/pkg/iterator"
	"runtime"
	"sync"
	"sync/atomic"
	"time"
...
type TCPF struct {
	tcpF       iterator.SimpleIterator[conf.TCPF]
	clientTCPF map[uint64]*iterator.SimpleIterator[conf.TCPF]
	mu         sync.RWMutex
}
...
func NewSendHandle(cfg *conf.Network) (*SendHandle, error) {
	...
	sh := &SendHandle{
		...
		tcpF:    TCPF{tcpF: iterator.SimpleIterator[conf.TCPF]{Items: cfg.TCP.LF}, clientTCPF: make(map[uint64]*iterator.SimpleIterator[conf.TCPF])},
...
func (h *SendHandle) setClientTCPF(addr net.Addr, f []conf.TCPF) {
	a := *addr.(*net.UDPAddr)
	h.tcpF.mu.Lock()
	h.tcpF.clientTCPF[hash.IPAddr(a.IP, uint16(a.Port))] = &iterator.SimpleIterator[conf.TCPF]{Items: f}
	h.tcpF.mu.Unlock()
}
