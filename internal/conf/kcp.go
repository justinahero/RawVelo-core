...
func (k *KCP) setDefaults(role string) {
	...
	// FEC پیش‌فرض: 10 data / 3 parity — مناسب اکثر لینک‌ها
	if k.Dshard == 0 {
		k.Dshard = 10
	}
	if k.Pshard == 0 {
	...
	if k.Smuxbuf == 0 {
		k.Smuxbuf = 16 * 1024 * 1024 // 16 MB
	}
	if k.Streambuf == 0 {
		k.Streambuf = 8 * 1024 * 1024 // 8 MB
	}
	if k.Smuxkalive_ == 0 {
		k.Smuxkalive_ = 5
	}
...
func (k *KCP) validate() []error {
	...
	validModes := []string{"normal", "fast", "fast2", "fast3", "extreme", "manual"}
	...
	if k.MTU < 50 || k.MTU > 1500 {
		errors = append(errors, fmt.Errorf("KCP MTU must be between 50-1500"))
	}
	...
	if k.Dshard < 0 || k.Dshard > 255 {
		errors = append(errors, fmt.Errorf("KCP dshard must be between 0-255"))
	}
	if k.Pshard < 0 || k.Pshard > 255 {
		errors = append(errors, fmt.Errorf("KCP pshard must be between 0-255"))
	}

	validBlocks := []string{"aes", "aes-128", "aes-128-gcm", "aes-192", "salsa20",
		"blowfish", "twofish", "cast5", "3des", "tea", "xtea", "xor", "sm4", "none", "null"}
	if !slices.Contains(validBlocks, k.Block_) {
		errors = append(errors, fmt.Errorf("KCP block must be one of: %v", validBlocks))
	}
	...
	if k.Smuxbuf < 1024 {
		errors = append(errors, fmt.Errorf("smuxbuf must be >= 1024 bytes"))
	}
	if k.Streambuf < 1024 {
		errors = append(errors, fmt.Errorf("streambuf must be >= 1024 bytes"))
	}
